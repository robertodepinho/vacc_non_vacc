...
output$plot_vacc_non_vacc_covid <- renderPlot({
  ...
  population_outcome() %>%
    ...
    geom_point(aes(
      ...
      fill = outcome), 
      size = point_size) +
    #geom_point(size = point_size, color = "black", shape = 10) +
    scale_shape_manual(values = c(24,25), breaks = c(T,F)) +
    scale_fill_manual(values = c("black", "green","grey","grey"), breaks = c("death", "hosp","infected", "none"))+
    scale_color_manual(values = c("red","grey"), breaks = c(T,F))+
    theme_void()
})
```
